...
의약품 검색을 통해 사용자 맞춤형으로 복약 관리를 할 수 있도록 제작하였다. 복약 관리 기능에서 사용자의 신체 상황에 따른 DUR 정보를 제공하여 복약 시 주의 사항을 숙지할 수 있도록 도와준다는 최종적인 목표를 담았으며, 이 애플리케이션을 사용하면 DUR 정보에 중점을 둔 맞춤형 복약 관리 기능으로 의약품 복약 시 따르는 위험성을 줄일 수 있을 것이다. 복약 관리 시스템을 가진 기존의 애플리케이션들은 사용법이 복잡하여 어렵다는 단점이 있다. 이 단점을 보완하기 위해 쉽게 사용할 수 있도록 만들었다. 단순한 사용법으로 모든 연령층이 이용할 수 있음을 기대할 수 있을 것이다.
